...
def test_circle_gauge_thickness_control_is_visible(qapp):
    panel = PropertiesPanel()
    panel.set_element(ThemeElement("circle_gauge", line_width=40))
    assert not panel.line_width_spin.isHidden()
    assert panel.line_width_spin.value() == 40
