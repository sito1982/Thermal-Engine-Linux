...
``ThemeEditorWindow.render_theme_image``.
"""
...
def _pil_renders(qapp, method_name, kind, font=None):
    ...
    method = getattr(ThemeEditorWindow, method_name)
    element = _element(kind)
    if font is None:
        method(None, image, element, 100, 100)
    else:
        method(None, image, element, font, 100, 100)
    return image
