...
def test_properties_hdmi_ranges_and_restore(qapp):
    panel = PropertiesPanel()
    panel.set_hdmi_mode(2560, 1440)
    assert panel.x_spin.maximum() == 2560
    assert panel.y_spin.maximum() == 1440
    assert panel.width_spin.maximum() == 2560
    assert panel.height_spin.maximum() == 1440
    assert panel.width_spin.minimum() == 10
    # Al volver a LCD se restauran los rangos clásicos
    panel.set_dmd_mode(False)
    assert panel.x_spin.maximum() == DISPLAY_WIDTH
...
def test_settings_defaults_include_hdmi():
    assert "hdmi" in DEFAULT_SETTINGS["project_targets"]
    assert DEFAULT_SETTINGS["project_targets"]["hdmi"] is False
    assert "hdmi_config" in DEFAULT_SETTINGS
